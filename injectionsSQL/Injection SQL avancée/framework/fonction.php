...
//fonction de protection légère face aux injections SQL
function sqlDetect(string $chaine): bool
{
	$filtre = ["select", "where", ";", "--", "=", "base64", "decode"];
	for ($i = 0; $i < count($filtre); $i++) {
		if (str_contains($chaine, $filtre[$i]))
			//Retourne true si présence de mots filtrés dans la chaine
			return true;
	}
	//retourne false si rien n'a été détecté
	return false;
}
